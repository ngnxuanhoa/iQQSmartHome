...
rqmoblie_nsp.use(middleware);						//Khi rqmoblie emit bất kỳ lệnh gì lên thì sẽ bị bắt
...
esp8266_nsp.on('connection', function(socket) {
	...
	socket.on("*", function(packet) {
		...
		rqmoblie_nsp.emit(eventName, eventJson) //gửi toàn bộ lệnh + json đến rqmoblie
	})
})
...
rqmoblie_nsp.on('connection', function(socket) {
	
	console.log('rqmoblie connected')
	
	//Khi webapp socket bị mất kết nối
	socket.on('disconnect', function() {
		console.log("Disconnect socket rqmoblie")
	})
